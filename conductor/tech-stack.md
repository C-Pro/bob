...
- **Sandbox Execution & Network Isolation:** Unprivileged Linux user namespaces (`bubblewrap` / `bwrap`) and pure-Go Docker Engine daemon client (`docker`) with strict kernel-level network namespace airgapping (`--unshare-all` / `NetworkMode: "none"`); injected statically linked `bob-proxy-fwd` helper utility bridging loopback TCP (`127.0.0.1:18080`) to host Unix domain sockets; in-process HTTP/CONNECT filtering proxy (`internal/sandbox/proxy.go`) with single-resolution DNS rebinding / TOCTOU prevention, RFC 7230/9110 hop-by-hop header removal, and mandatory CIDR blocklists.
