...
- **Isolated Sandbox Execution Subsystem:** Secure, disposable sandbox execution via unprivileged Bubblewrap (`bwrap`) or pure-Go Docker Engine API (`docker`), strictly scoped to 1-on-1 Direct Messages with interactive user authorization cards (`/sandbox approve` / `/sandbox deny`), kernel-enforced network namespace airgapping, domain-filtering proxy with DNS rebinding / TOCTOU protection and hop-by-hop header sanitization, and dynamically resolved per-user workspace isolation.
