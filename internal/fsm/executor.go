...
func NewStepExecutor(invoker ToolInvoker, opts ...StepExecutorOption) *StepExecutor {
	e := &StepExecutor{
		invoker:     invoker,
		maxWorkers:  4,
	...
	for _, step := range steps {
		...
		go func(s *FSMStep) {
			...
			case <-ctx.Done():
				...
				s.CompletedAt = &now
				if err := e.updateStep(ctx, store, s); err != nil {
					recordErr(err)
				}
				recordErr(ctx.Err())
			}
...
func (e *StepExecutor) executeSequential(ctx context.Context, store StepStore, steps []*FSMStep) error {
	for i, step := range steps {
		...
		if ctx.Err() != nil {
			...
			if err := e.updateStep(ctx, store, step); err != nil {
...
func (e *StepExecutor) skipRemainingSteps(ctx context.Context, store StepStore, steps []*FSMStep, failedStepID string) error {
	now := time.Now().Unix()
	var errs error
	for _, s := range steps {
...
func (e *StepExecutor) executeSingleStep(ctx context.Context, store StepStore, step *FSMStep) error {
	...
	for {
		...
		if ctx.Err() != nil {
			...
			step.CompletedAt = &completedTime
			updateErr := e.updateStep(ctx, store, step)
